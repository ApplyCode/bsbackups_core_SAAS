...
function Index(props) {
  ...
  /* TITLE PAGE */
  document.title = "Planos | BS Backups"

  useEffect(() => {
    async function fetchAll() {
      dispatch(await getPlans())
      setLoading(false)
    }
    fetchAll()
  }, [])
    ...
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <Breadcrumbs
            title="BS Backups"
            breadcrumbItem="Planos"
            btn="Novo Plano"
            btnRedirect="/plans/create"
          />
          <Card>
            <CardBody>
              <h4 class="card-title mb-4">{props.t("Planos")}</h4>
              {list.length > 0 ? (
                <>
                  <MDBDataTable
                    responsive
                ...
                <>
                  <Alert color="danger" className="mb-0" role="alert">
                    ...
                    Não existe nenhum plano cadastrada no momento.
                  </Alert>
                </>
              )}
            </CardBody>
